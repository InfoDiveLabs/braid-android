...
interface Phone {
    ...
    /**
     * Asks a person whether [desktop] may use this phone, blocking until they
     * answer or give up. Returns the new key, or null for refusal.
     */
    fun pair(desktop: String): String?
}
...
class ControlPlane(private val phone: Phone) {
    fun respond(head: RequestHead, path: String, body: ByteArray): Reply {
        ...
        return when (route) {
            ...
            "/braid/status" -> ok(Json.obj("lanes" to Json.array(phone.lanes().map(::encode))))
            else -> pair(body)
        }
    }

    private fun pair(body: ByteArray): Reply {
        val desktop = Json.parseObject(body.toString(Charsets.UTF_8))?.get("desktop") as? String
            ?: return Reply(400, "Bad Request")
        val key = phone.pair(desktop) ?: return Reply(403, "Forbidden")
